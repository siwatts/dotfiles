...
"let terminal_theme='gruvbox'
"let gui_theme='gruvbox'
...
function! SetCustomDiff()
    if !has("gui_running") && ( $TERM == 'linux' || $TERM == 'cygwin' || &t_Co < 256 )
        ...
        hi DiffAdd      ctermfg=7       ctermbg=1   cterm=bold  term=bold
        hi DiffDelete   ctermfg=0       ctermbg=4   cterm=NONE  term=bold
        hi DiffChange   ctermfg=7       ctermbg=5   cterm=bold  term=bold
        hi DiffText     ctermfg=7       ctermbg=1   cterm=bold  term=reverse
    else
